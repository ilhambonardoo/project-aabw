...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function hitungTotal() {
        ...
        document.getElementById('totalDebit').value = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(totalDebit);
        document.getElementById('totalKredit').value = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(totalKredit);

        const ketBalance = document.getElementById('keteranganBalance');
        if (totalDebit === totalKredit && totalDebit > 0) {
            ketBalance.innerHTML = '<span class="text-success"><i class="bi bi-check-circle-fill"></i> Seimbang</span>';
        } else {
            ketBalance.innerHTML = '<span class="text-danger"><i class="bi bi-x-circle-fill"></i> Tidak Seimbang</span>';
        }
    }
