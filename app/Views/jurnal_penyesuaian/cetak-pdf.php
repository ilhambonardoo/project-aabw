...
<head>
    ...
    <?php $penyesuaianStyle = FCPATH . '/css/cetak-jurnal.css'; 
        if(file_exists($penyesuaianStyle)){
            echo '<style>'. file_get_contents($penyesuaianStyle) .  '</style>';
        }
    ?>
</head>
<body>
    <div class="container">
        ...
        <table>
            ...
            <tbody>
                <?php if (empty($jurnal)): ?>
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 20px;">Tidak ada data jurnal penyesuaian</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($jurnal as $noTransaksi => $transaksi): ?>
                        ...
                        <?php foreach ($transaksi['details'] as $detail): ?>
                            <tr class="detail-row">
                                <td></td>
                                <td style="padding-left: 30px;"><?= $detail['kode_akun'] ?> - <?= $detail['nama_akun'] ?></td>
                                <td></td>
                                <td class="text-right">
        ...
        <div class="footer">
            <div class="signature">
                <div class="signature-date">
                    Bogor, <?= date('d F Y', strtotime($tanggal_cetak)) ?>
                </div>
